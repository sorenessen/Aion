#!/usr/bin/env python3
"""Create a source-only recovery snapshot of the current Aion working tree."""
...
destination = root.parent / "Aion-recovery.zip"

with zipfile.ZipFile(destination, "w", zipfile.ZIP_DEFLATED) as archive:
    for name in files:
        path = root / name
        if path.is_file() and not path.is_symlink():
            archive.write(path, arcname=f"Aion/{name}")
    archive.writestr("Aion/RECOVERY_STATUS.txt", f"Branch: {branch}\nCommit: {commit}\n\nGit status:\n{status}")
